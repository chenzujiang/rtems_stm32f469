...
#if defined(USART_DRIVER_USE_INTERRUPT) && defined(USART_DRIVER_USE_DMA)
 #error "Can't defind interrupt and dma at the same time"
#endif
...
#ifdef USART_DRIVER_USE_DMA
static void  usart_dma_irq(int iflags, void *arg)
{
  int  minor = (int)arg;
  console_data  *cdata = Console_Port_Data + minor;
  int  size = (int)cdata->pDeviceContext;
...
#ifdef USART_DRIVER_USE_DMA
  dma_config  dma_cfg = {{0}};
  int  dma_ch = Console_Port_Tbl [minor]->ulDataPort;
#endif
...
#endif
#ifdef USART_DRIVER_USE_DMA
  dma_cfg.reg.dma_tcie = true;  /* Transmit complete interrupt */
  dma_cfg.reg.dma_dir  = 1;     /* Memory to periph */
  dma_cfg.reg.dma_pinc = false;
  dma_cfg.reg.dma_minc = true;
  dma_cfg.reg.dma_psize = 0;  /* Byte */
  dma_cfg.reg.dma_msize = 0;
  dma_cfg.reg.dma_pl = 0;  /* Priority is low */
  dma_cfg.reg.dma_ct = 0;
  dma_cfg.reg.dma_mburst = 1;
  dma_cfg.reg.dma_pburst = 1;
  dma_cfg.reg.dma_chsel = STM32F4_GET_CHANNEL(dma_ch); /* DMA channel select */

  stm32f4_dma_init(dma_ch, &dma_cfg, 1 << 2);
  dma_set_interrupt_handler(
          dma_ch, 
          usart_dma_irq, 
          (void *)minor
  );
  usart->cr3 = STM32F4_USART_CR3_DMAT | \
...
static int usart_interrupt_write(int minor, const char *d, size_t n)
{
  ...
  cdata->pDeviceContext = (void *)n;
  dma_copy(dma_ch, d, &usart->dr, n, DMA_MEM_TO_PERIPHERY);
#endif
  return n;
}

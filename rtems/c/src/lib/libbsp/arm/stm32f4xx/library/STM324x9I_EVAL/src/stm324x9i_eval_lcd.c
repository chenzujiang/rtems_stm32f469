...
#define BUFFER_OFFSET          ((uint32_t)0x177000)
...
void LCD_Config(void)
{
    ...
    RCC_APB1PeriphClockCmd(RCC_AHB1Periph_DMA2D, ENABLE);
  ...
  LTDC_Layer_InitStruct.LTDC_HorizontalStop = HSYNCH + HBP - 1 + LCD_PIXEL_WIDTH; 
  LTDC_Layer_InitStruct.LTDC_VerticalStart = VSYNCH + VBP + 0;
  LTDC_Layer_InitStruct.LTDC_VerticalStop = VSYNCH + VBP - 1 + LCD_PIXEL_HEIGHT;
...
void DSI_PPL_Structure_Configuration(void)
{
    DSI_InitTypeDef DSI_InitStructure;
    DSI_PLLInitTypeDef PLLInitStructure;
    PLLInitStructure.PLLNDIV = 125;
    PLLInitStructure.PLLIDF = DSI_PLL_IN_DIV2;
...
void DSI_Cmd_Transmission_Mode_conf(void)
{
...
void DSI_DSISync_INT_conf(void)
{
...
void DSI_Vido_Mode_conf(void)
{
  ...
  /* Value depending on display orientation choice portrait/landscape */
...
void DSI_Config(void)
{
   ...
   /* Enable the LTDC clock */
    ...
    //LCD_AF_GPIOConfig();
    LCD_I2C_Config();      
    /* Check if the LCD is AMPIRE480272 or AMPIRE640480 USE_LED_OTM800480*/
    LCD_CheckDevice();
    /* RCC Configuration -------------------------------------------------------*/
...
{    
  ...
  IOE_Config();
  if(IOE_ReadID() == (uint16_t)TF6206_VALUE)
  {
     CurrentLcd = USE_LED_OTM800480;  
  }
  else if(IOE_ReadID() == (uint16_t)STMPE811_ID)
  {
